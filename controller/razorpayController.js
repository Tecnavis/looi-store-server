...
exports.order = async(req, res) => {
    try {
        ...
        // FIX: amount must be in paise (integer). Frontend sends totalAmount * 100
        if (!amount || isNaN(amount) || Number(amount) <= 0) {
            return res.status(400).json({ success: false, message: 'Invalid amount. Send amount in paise (rupees x 100).' });
        }

        const options = {
            amount: Math.round(Number(amount)), // integer paise
...
exports.validate = async(req, res) => {
    ...
    if (dbOrderId) {
        ...
        try {
            await Order.findOneAndUpdate(
                { razorpayOrderId: razorpay_order_id },
                { paymentStatus: 'Paid', orderStatus: 'Processing', razorpayPaymentId: razorpay_payment_id }
            );
        } catch (err) {
    ...
    res.json({ success: true, order: { orderId: razorpay_order_id, paymentId: razorpay_payment_id } });
};
